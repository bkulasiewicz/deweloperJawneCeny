...
class DeweloperJawneCeny {
    ...
    private function load_dependencies() {
        ...
        DIContainer::init();
        Logger::info('Main: DI Container initialized successfully');

        // Initialize context-specific components
        if (is_admin()) {
            Logger::info('Main: Initializing admin context');
            $this->init_admin();
        }

        Logger::info('Main: Initializing frontend context');
        $this->init_frontend();
    ...
    /**
     * Load only essential dependencies for plugin activation (without DI52)
     */
    private function load_activation_dependencies() {
        // WordPress filesystem functions
        require_once(ABSPATH . 'wp-admin/includes/file.php');

        // Helpers (Logger is essential for activation)
        require_once PLUGIN_DIR . 'includes/helpers/PremiumHelper.php';
        require_once PLUGIN_DIR . 'includes/helpers/Logger.php';
        ...
        // Only load SettingsRepository for activation
        require_once PLUGIN_DIR . 'includes/repositories/SettingsRepository.php';
        require_once PLUGIN_DIR . 'includes/repositories/DeveloperRepository.php';
        require_once PLUGIN_DIR . 'includes/repositories/InvestmentRepository.php';
        require_once PLUGIN_DIR . 'includes/repositories/ResourceRepository.php';
        ...
        Logger::info('UJC: Activation dependencies loaded (without DI52)');
    }

    public function activate() {
        // ULTRA MINIMAL ACTIVATION - tylko podstawowe error logging
        error_log('UJC: MINIMAL activation start');

        try {
            error_log('UJC: Testing basic functionality');

            // Test 1: Sprawdź czy WordPress functions działają
            $upload_dir = wp_upload_dir();
            if ($upload_dir) {
                error_log('UJC: wp_upload_dir() works');
            }

            error_log('UJC: Basic tests passed');

        } catch (Exception $e) {
            error_log('UJC: Exception during activation: ' . $e->getMessage());
        } catch (Error $e) {
            error_log('UJC: Fatal error during activation: ' . $e->getMessage());
        }

        error_log('UJC: MINIMAL activation completed');
    }
    ...
    private function init_admin() {
        ...
        Logger::info('Main: Admin context initialized with AdminController from DI Container');
    }
    ...
    private function init_frontend() {
        ...
        Logger::info('Main: Frontend context initialized with DI Container');
    }
